...
case class WikiEditEvent(timeStamp:Long,
                              channel:String,
                              title:String,
                              diffUrl:String,
                              user:String,
  ...
  flag = WikiEditEventUtil.getFlags(isMinor,
    isNew,
    isUnpatrolled,
    isBotEdit,
    isSpecial,
...
object WikiEditEventUtil {
  ...
  def fromRawEvent(timeStamp:Long, channel:String, rawEvent:String): Option[WikiEditEvent] = {
    ...
    if (matcher.find() && matcher.groupCount() == 6) {
      ...
      Some(WikiEditEvent(timeStamp, channel, title, difUrl,
