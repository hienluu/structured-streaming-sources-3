...
class WikiEditChannelListener(editQueue:BlockingQueue[WikiEditEvent]) extends IRCEventListener {
  val logger:Logger = LoggerFactory.getLogger(this.getClass)

  override def onPrivmsg(target: String, ircUser: IRCUser, message: String) = {

    val event:Option[WikiEditEvent] = WikiEditEventUtil.fromRawEvent(
